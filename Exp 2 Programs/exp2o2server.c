...
int main()
{
    ...
    listen(listenfd,5); connfd=accept(listenfd,(struct sockaddr *)&cliaddr ,&len);
    n=read(connfd , buff ,sizeof(buff)); 
    buff[n]=0; 
    int num = atoi(buff);
    n=read(connfd , data ,sizeof(data)); 
    data[n]=0;
    int arr[num], i = 0; char* temp = strtok(data, " "); while (temp != NULL)
    {
    arr[i++] = atoi(temp); temp = strtok(NULL, " ");
    }
